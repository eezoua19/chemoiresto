...
(async () => {
  ...
  check('Mauvais mot de passe refuse', badLogin.status === 401);
  ...
  // 4. Menu du jour cote client
  const menu = await api(`/api/menu/table/${table.token}`);
  check('Menu du jour recupere', menu.success && menu.data.menu && menu.data.menu.items.length > 0);
  ...
  // 5. Socket.IO : la serveuse ecoute
  ...
  // 6. Commande client - le total doit etre recalcule par le serveur
  ...
  check('Commande creee', order.success, order.message);
  check('Numero au format CMD-AAAAMMJJ-NNNN', /^CMD-\d{8}-\d{4}$/.test(order.data.orderNumber), order.data.orderNumber);
  ...
  check('Socket.IO : new_order recu par la serveuse', received.orderNumber === order.data.orderNumber);
  ...
  check('Transition invalide refusee (SERVED -> PREPARING)', badTransition.status === 400);

  await new Promise((r) => setTimeout(r, 500));
  check(
    'Client a recu les 4 changements en temps reel',
    ['ACCEPTED', 'PREPARING', 'READY', 'SERVED'].every((s) => clientEvents.includes(s)),
    clientEvents.join(', ')
  );

  // 9. Attribution automatique a la serveuse
  const detail = await api(`/api/orders/${orderId}`, { token: adminToken });
  check('Commande attribuee automatiquement a Marie', detail.data.server && detail.data.server.firstName === 'Marie');
  ...
  if (outside) {
    ...
    check('Produit hors menu du jour refuse', refused.status === 400, refused.message);
  }

  // 12. Prix falsifie ignore (le frontend n'envoie pas de prix, mais on verifie
  ...
  // Table dediee : l'anti-spam reutiliserait sinon une demande deja ouverte
  // sur la table 01 et n'emettrait aucun evenement.
  ...
  check('Socket.IO : service_request recu', callEvent.type === 'CALL_SERVER');
  ...
  check('Demande d\'addition envoyee', bill.success && bill.data.status === 'REQUESTED');
  ...
  check('Addition marquee payee', billDone.success && billDone.data.status === 'PAID');
  ...
  check('Menu du 24/12 cree', menu1.success, menu1.message);
  check('Prix special du jour enregistre', menu1.data.items.find((i) => i.productId === poulet.productId).price === 4500);
  ...
  check('Menu copie vers le 25/12', dup.success, dup.message);
  ...
  check('Menu source inchange apres copie', source.data.menu.items.length === 2);
  ...
  check('Doublon de date refuse', conflict.status === 409);
  ...
  check('Table creee avec QR Code', newTable.success && newTable.data.qrCode);
  ...
  check('Table desactivee bloque le menu', inactive.status === 403, inactive.message);
  ...
  check('Notifications enregistrees', notifs.success && notifs.data.length > 0, `${notifs.data.length} notification(s)`);
