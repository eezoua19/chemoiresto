...
export function CardHeader({ title, subtitle, action, icon: Icon }) {
  return (
    <div className="flex items-start justify-between gap-4 border-b border-ink-100 dark:border-ink-700 px-5 py-4">
      <div className="flex items-start gap-3">
        {Icon && (
          <span className="mt-0.5 rounded-lg bg-brand-50 dark:bg-brand-900/30 p-2 text-brand-600">
            <Icon size={18} />
          </span>
        )}
        <div>
...
export function Field({ label, error, hint, required, children }) {
  return (
    <div>
      ...
      {children}
      {hint && !error && <p className="mt-1 text-xs text-ink-500 dark:text-ink-400">{hint}</p>}
...
export function Toggle({ checked, onChange, label, disabled }) {
  return (
    ...
    >
      ...
      >
        <span
          className={`absolute top-0.5 h-5 w-5 rounded-full bg-white dark:bg-ink-800 shadow transition-all ${
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center">
      ...
      <div
        ...
        className={`relative flex max-h-[92vh] w-full ${sizes[size]} flex-col rounded-t-2xl
                    bg-white dark:bg-ink-800 shadow-float sm:rounded-2xl ${
                      sortant
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-ink-100 dark:border-ink-700 px-5 py-4">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg p-1.5 text-ink-400 dark:text-ink-500 transition hover:bg-ink-100 dark:hover:bg-ink-700 hover:text-ink-700 dark:hover:text-ink-200"
            aria-label="Fermer"
          >
            <X size={20} />
          </button>
        ...
        {footer && (
          <div className="flex flex-wrap justify-end gap-2 border-t border-ink-100 dark:border-ink-700 px-5 py-4">
            {footer}
          </div>
        )}
      </div>
...
export function EmptyState({ icon: Icon = Inbox, title, description, action }) {
  return (
    <div className="flex flex-col items-center justify-center gap-3 px-6 py-14 text-center">
      <span className="rounded-2xl bg-ink-100 dark:bg-ink-800 p-4 text-ink-400 dark:text-ink-500">
        <Icon size={28} />
      </span>
      <div>
        <h3 className="text-base font-semibold text-ink-800 dark:text-ink-100">{title}</h3>
        {description && <p className="mx-auto mt-1 max-w-sm text-sm text-ink-500 dark:text-ink-400">{description}</p>}
      </div>
      {action}
    </div>
  );
  ...
  return (
    <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-ink-900 dark:text-ink-50 sm:text-2xl">{title}</h1>
...
export function StatCard({ label, value, icon: Icon, tone = 'brand', hint }) {
  const tones = {
    ...
    emerald: 'bg-emerald-50 dark:bg-emerald-900/20 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400',
    amber: 'bg-amber-50 dark:bg-amber-900/20 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400',
    sky: 'bg-sky-50 dark:bg-sky-900/20 dark:bg-sky-900/30 text-sky-600 dark:text-sky-400',
    indigo: 'bg-indigo-50 dark:bg-indigo-900/20 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400',
  ...
  return (
    <div className="card flex items-center gap-4 p-4">
      ...
      <div className="min-w-0">
        ...
        <p className="mt-0.5 truncate text-xl font-bold text-ink-900 dark:text-ink-50">{value}</p>
        {hint && <p className="truncate text-xs text-ink-400 dark:text-ink-500">{hint}</p>}
      </div>
    </div>
  );
}
...
export function Footer({ className = '' }) {
  ...
  return (
    <footer className={`px-4 py-5 text-center text-xs text-ink-600 dark:text-ink-300 ${className}`}>
      <p>
