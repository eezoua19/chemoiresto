        ...
        {error ? (
          ...
          <div className="p-3 sm:p-5">
            <div className="mb-2 grid grid-cols-7 gap-1.5 sm:gap-2">
              {WEEKDAYS.map((day) => (
                <div key={day} className="py-1 text-center text-xs font-bold uppercase text-ink-400">
                  {day}
                </div>
              ))}
            </div>
                ...
                return (
                  <button
                    ...
                    className={`flex aspect-square flex-col items-center justify-center gap-1 rounded-xl border p-1 transition
                      ${isToday ? 'border-brand-400 bg-brand-50' : 'border-ink-100 bg-white hover:border-brand-200 hover:bg-ink-50'}
                      ${isPast && !hasMenu ? 'opacity-50' : ''}`}
                  >
                    <span
                      className={`text-sm font-bold ${isToday ? 'text-brand-700' : 'text-ink-800'}`}
                    >
                      {date.getDate()}
                    </span>
                    ...
                    {hasMenu && (
                      <span className="hidden text-[10px] font-medium text-ink-500 sm:block">
            ...
            <div className="mt-5 flex flex-wrap gap-5 border-t border-ink-100 pt-4 text-sm text-ink-600">
